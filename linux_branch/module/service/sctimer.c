...
// 先创建链表对象处理函数
static struct stnode* _new_stnode(int start, int cut, int intval, die_f timer, void* arg, bool fb)
{
	int s = start / 1000;
	...
	node->cut = cut > 0 ? cut + 1 : 0; // 执行到1的时候停止,并且兼容永久时间0
	node->intval = intval;
	node->type = fb;
	node->timer = timer;
	node->arg = arg;
 ...
 */
int 
st_add(int start, int cut, int intval, die_f timer, void* arg, bool fb)
{
	struct stnode* now;
	DEBUG_CODE({
		if(start<0 || cut<0 || intval<0 || !timer)
			CERR_EXIT("debug start,cut,intval,timer => %d,%d,%d,%p.", start, cut, intval, timer);
	});
	// 这里开始创建对象往 线程队列中添加
	now = _new_stnode(start, cut, intval, timer, arg, fb);
